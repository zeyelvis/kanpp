 ...
 * that TMDB does not list. The rules (corroboration by two sources, synopsis, name policy)
 * live in lib/ingest/source-titles.ts.
 *
 *   npx tsx scripts/source-titles.ts --db=remote --dry-run
 *   npx tsx scripts/source-titles.ts --db=remote [--limit=500]
 *
 * Everything is computed locally and written in batches with explicit title ids, so nothing
 * else may create titles meanwhile: the run holds the same lock as ops/run-ingest.sh (pass
 * --no-lock when called from inside it).
 */
...
import { storeCatalogCounts, refreshTitles } from "@/lib/ingest/publish";
import { groupSourceRows, planSourceTitle, SOURCE_TITLE_TYPES, type SourceRow, type SourceTitlePlan } from "@/lib/ingest/source-titles";
...
const LOCK = "/tmp/kanpp-ingest.lock";
...
async function fillContent(db: Db, rows: SourceRow[]): Promise<number> {
    ...
    [...bySource.entries()].map(async ([sourceId, list]) => {
      ...
      await chunks(list, 20, async (chunk) => {
        const items = await fetchCmsByIds(source, chunk.map((r) => r.vod_id)).catch((err: unknown) => {
          log(`details ${sourceId} failed: ${err instanceof Error ? err.message : err}`);
          return [];
        });
...
async function main() {
  ...
  log(`synopses fetched: ${await fillContent(db, needSynopsis.flatMap((g) => g.rows))}`);
  ...
  if (args.limit) fresh.length = Math.min(fresh.length, Number(args.limit));
  ...
  // Explicit title ids from here on: nothing else may create titles until the batches are in.
  const locked = args.lock && target !== "local";
  if (locked) {
    try {
      mkdirSync(LOCK);
    } catch {
      throw new Error(`${LOCK} exists: an ingest run is active, try again later`);
    }
  }
  try {
    await write(db, target, fresh, attach);
  } finally {
    if (locked) rmdirSync(LOCK);
  }
}

async function write(db: Db, target: string, fresh: SourceTitlePlan[], attach: { titleId: number; plan: SourceTitlePlan }[]) {
  ...
  log(`revalidate: ${await notifySite({ titleIds: [...new Set(attach.map((a) => a.titleId))], created: created.length > 0, catalog: true })}`);
  log(`indexnow: ${await submitIndexNow(created.map((c) => titlePath(c.kind, c.slug)))}`);
    ...
    Array.from({ length: 6 }, async () => {
      for (let c = queue.shift(); c; c = queue.shift()) {
        ...
        for (let attempt = 0; attempt < 2 && !ok; attempt++) {
          const res = await fetch(`${site.url}/img${c.poster}`).catch(() => null);
          ok = Boolean(res?.ok);
  ...
  if (broken.length) {
    ...
    await notifySite({ titleIds: broken, created: false, catalog: true });
  }
